    ...
    def plot_rooms(self, rooms):
        ...
        for room in rooms:
            ...
            if(room['name'] == 'Bedroom 1'):
                ...
                print("done")
                # add bed
                ax.add_patch(Rectangle((position[0], position[1]+0.15*size[1]), 0.75*size[0], 0.65*size[1], fill=False, color='black',linestyle='dotted'))

                # add pillows on bed
                ax.add_patch(Rectangle((position[0]+0.05*size[0], position[1]+0.2*size[1]), 0.1*size[1], 0.2*size[0], fill=False, color='black'))
                ax.add_patch(Rectangle((position[0]+0.05*size[0], position[1]+0.5*size[1]), 0.1*size[1], 0.2*size[0], fill=False, color='black'))
...
if __name__ == '__main__':
    while True:
        ...
        print(population)
        population = [[{'name': 'Living Room', 'position': (0, 0), 'size': (40, 40)}, {'name': 'Bedroom 1', 'position': (0,60), 'size': (50, 40)}, {'name': 'Kitchen', 'position': (0,40), 'size': (40, 20)}, {'name': 'door', 'position': (0, 0), 'size': (5, 5)},{'name': 'toilet', 'position': (40,40), 'size': (10, 20)}]]                      
